<!-- 
ind 0 -> tworzenie ind dziecka(parents: 1) -> tworzenie ind małżonka ->  tworzenie rodziny 1 w families (father: ja | mother: małżonek)
rodzina 0 
 -->
 <!DOCTYPE html>
<html lang="en">
...
<body>
 ...
 <?php
    $conn = new mysqli('localhost', 'root', '', 'familytree3');
    ...
    if($conn->query("SELECT * FROM individuals")->num_rows < 1) {               //jeśli nie ma rekordów dodajemy pierwszą osobę
        ...
        $conn->query("ALTER TABLE families AUTO_INCREMENT = 1;");
        $conn->query("INSERT INTO individuals VALUES (NULL, 'First Name','Last Name', 0, 'm','".date("Y-m-d")."')");
        $conn->query("INSERT INTO families VALUES (NULL, 1, 1)");
    }
